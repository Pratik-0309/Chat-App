 ...
 useEffect(() => {
  ...
  return selectedUser ? (
    <div className="h-full flex flex-col overflow-hidden relative backdrop-blur-lg">
      ...
      {/* Chat Area */}
      <div className="flex flex-col h-full overflow-y-auto p-3 pb-28">
          ...
          >
            {msg.image ? (
              <img
                src={msg.image}
                alt="image"
                className="max-w-57.5 border border-gray-700
              rounded-lg overflow-hidden mb-8"
              />
            ) : (
              <p
                className={`p-2 max-w-50 md:text-sm font-light rounded-lg mb-8 break-all
                bg-violet-500/30 text-white ${msg.senderId !== "680f50e4f10f3cd28382ecf9" ?
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3 bg-black/30">
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
          <input type="text" placeholder="Send a message" className="flex-1 text-sm p-3 border-none
          rounded-lg outline-none text-white placeholder-gray-400" />
