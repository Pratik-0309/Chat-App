    ...
    ${selectedUser ? "max-md:hidden" : ""}`}>
      <div className="pb-5">
        <div className="flex justify-between items-center">
          <img src={assets.logo2} alt="" className='max-w-40' />
          <div className='relative py-2 group'>
          <img src={assets.menu_icon} alt="" className='max-w-5 cursor-pointer' />
          <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#281242] border
      ...
      <div className='flex flex-col'>
        {userDummyData.map((user,index)=> (
          ...
          max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            ...
            {index > 2 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex 
            justify-center items-center rounded-full bg-violet-500/50'>{index}</p>}
          </div>
        ))}
      </div>
    </div>
